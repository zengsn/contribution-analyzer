	...
	public static void run() {
					...
					File jsonFile3 = new File(file.getAbsolutePath() + "/data/repo" + "/" + name1);
						...
						if (!jsonFile1.exists()) {
							...
							File jsonFile = new File(
									file.getAbsolutePath() + "/data/repo" + "/" + reponame + "/" + reponame + ".json");
        ...
        if (files1 != null) {
			for (int i = 0; i < files1.length; i++) {
				...
				File jsonFile5 = new File(
						file.getAbsolutePath() + "/data/repo" + "/" + reponame + "/" + "contributors.json");
				...
				if (files2 != null) {
					for (int a = 0; a < files2.length; a++) {
						...
						if (!cName.equals("contributors.json") & !cName.equals("AllPulls")) {
							...
							File jsonFile7 = new File(
									file.getAbsolutePath() + "/data/repo" + "/" + reponame5 + "/AllPulls");
							if (!jsonFile7.exists()) {
								String pullurl = url.replace("{/number}", "");
								String purl = pullurl + "?" + STATE_CLOSED + "&" + CLIENT_ID_SECRET + "&" + PER_PAGE
										+ "20&" + PAGE;
								...
								if (!jsonFile7.exists()) {
									jsonFile7.mkdirs();
									File jsonFile6 = new File(file.getAbsolutePath() + "/data/repo" + "/" + reponame5
											+ "/" + "AllPulls/" + "allpull.json");
						...
						if (files2.length < 3) {
							for (int b = 0; b < files2.length; b++) {
								...
								if (!files2[b].getName().equals("follower.json")) {
									...
									int pages = 1;
									String resp = Http.get(purl + pages);
									if (resp == null) {throw new RuntimeException("Take a rest for one hour!"); // 限制到了，应该停一个小时
                                     }
									while (resp != null && resp.startsWith("[")) {
										List<Map<String, Object>> list = JsonUtils.toList(resp);
										if (list.size() > 0) {
					...
					for (int a = 0; a < files16.length; a++) {
						...
						if (files2.length < 4) {
							for (int b = 0; b < files2.length; b++) {
								List<Map<String, Object>> pulls = new ArrayList<Map<String, Object>>();
								if (!files2[b].getName().equals("follower.json")
										& !files2[b].getName().equals("following.json")) {
									String user = files2[b].getAbsolutePath();
									File userFile3 = new File(user);
                                    String jsonFile1 = readFileToString(userFile3);
									Map<String, Object> usermap = JsonUtils.toMap(jsonFile1);
